...
const chips = createMenuItem("chips", 1, "lunch");

console.log("New Menu Items: ", pizza, fries, chips);
...
const burger = {
  ...
  discount: function (string) {
    if (string === "teacher" || string === "student") {
      const discountPrice = this.price * 0.75;
      return discountPrice;
    } else {
      const discountPricePublic = this.price * 0.9;
      return discountPricePublic;
    }
  },
};
...
function getReviewByRating(array, ratingReceived) {
  const newArray = array.filter(
    (review) => ratingReceived === Math.floor(review.rating)
  );
  return newArray;
}
...
function getLongReviews(array) {
  const longReviews = array.filter(
    (review) => review.feedback.split(" ").length > 15
  );
  return longReviews;
}
